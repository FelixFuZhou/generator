 ...
 * @version 2013-10-15
 */
public class GenTableColumn extends GenDataEntity<GenTableColumn> {
    ...
    private String            isNew;                //判断是否是子类的字段  id create_by这些不是新的 
    ...
    public String getSimpleJavaType(){
        if ("This".equals (getJavaType ())) { return StringUtils.capitalize (genTable.getClassName ()); }
    ...
    public List<String> getAnnotationList(){
        ...
        if (!"1".equals (getIsNull ()) && !"String".equals (getJavaType ())) {
            ...
            list.add ("org.hibernate.validator.constraints.Length(min=1, max=" + getDataLength () + ", message=\"" + getComments () + "长度必须介于 1 和 "
                    + getDataLength () + " 之间\")");
        } else if ("String".equals (getJavaType ()) && !"0".equals (getDataLength ())) {
            list.add ("org.hibernate.validator.constraints.Length(min=0, max=" + getDataLength () + ", message=\"" + getComments () + "长度必须介于 0 和 "
                    + getDataLength () + " 之间\")");
        }
        return list;
    }
    ...
    public Boolean getIsNotBaseField(){
        return !StringUtils.equals (getSimpleJavaField (), "id") && !StringUtils.equals (getSimpleJavaField (), "remarks")
                && !StringUtils.equals (getSimpleJavaField (), "createBy") && !StringUtils.equals (getSimpleJavaField (), "createDate")
                && !StringUtils.equals (getSimpleJavaField (), "updateBy") && !StringUtils.equals (getSimpleJavaField (), "updateDate")
                && !StringUtils.equals (getSimpleJavaField (), "delFlag");
    }
    
    /**
     * 获取简写Java类型 判断是否是string类型
     * @return
     */
    public boolean getIsString(){
        if(getSimpleJavaType ().equalsIgnoreCase ("String")){
            return true;
        }
        return false;
    }
